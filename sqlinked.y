...
void dblock_parse(astnode_t *root, astnode_t *func);
  ...
  switch (root->type) {
    ...
    case funcall:
      ...
      prog_add_num(p, nrparams);
            ...
            if (!f) {
//                vmerror(E_DEBUG2, NULL, "Undeclared function: %s", root->child[0]->v.id);
            } else {
                root->dt = f->dt;
                root->sdt = f->sdt;
            }
    ...
    case dblock:
      prog_add_op(p, TSTART);
      dblock_parse(root->child[0], curfunc);
      prog_add_op(p, TEND);
      prog_add_num(p, 0);
      break;
    ...
    case stmts:
      ...
      break;
        ...
        case _if:
            ...
            if (root->child[0]->dt == T_REC || root->child[0]->dt == T_STR || root->child[0]->dt == T_UNDEF) {
...
void dblock_parse(astnode_t *root, astnode_t *func) {
    ...
    char blockname[100];
    char *funcname = func ? func->child[0]->v.id : "global";
    snprintf(blockname, sizeof(blockname), "dblock_%s_%s_%d", p->name, funcname, dblockid);
